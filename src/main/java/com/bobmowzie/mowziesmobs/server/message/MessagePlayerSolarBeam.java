...
public class MessagePlayerSolarBeam {
    ...
    public static void serialize(final MessagePlayerSolarBeam message, final PacketByteBuf buf) {
    }

    public static MessagePlayerSolarBeam deserialize(final PacketByteBuf buf) {
    ...
    public static class Handler implements BiConsumer<MessagePlayerSolarBeam, Supplier<NetworkEvent.Context>> {
        ...
        public void accept(final MessagePlayerSolarBeam message, final Supplier<NetworkEvent.Context> contextSupplier) {
            ...
            context.enqueueWork(() -> {
                EntitySolarBeam solarBeam = new EntitySolarBeam(EntityType.get(), player.level(), player, player.getX(), player.getY() + 1.2f, player.getZ(), (float) ((player.yHeadRot + 90) * Math.PI / 180), (float) (-player.getXRot() * Math.PI / 180), 55);
