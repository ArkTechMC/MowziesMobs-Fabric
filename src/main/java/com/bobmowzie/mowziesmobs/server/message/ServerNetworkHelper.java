...
public class ServerNetworkHelper {
    public static void register() {
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.LEFT_MOUSE_DOWN, (server, player, handler, buf, responseSender) -> server.execute(() -> {
        ...
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.LEFT_MOUSE_UP, (server, player, handler, buf, responseSender) -> server.execute(() -> {
            PlayerCapability.IPlayerCapability capability = PlayerCapability.get(player);
            if (capability != null) {
        ...
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.RIGHT_MOUSE_DOWN, (server, player, handler, buf, responseSender) -> server.execute(() -> {
            PlayerCapability.IPlayerCapability capability = PlayerCapability.get(player);
            if (capability != null) {
        ...
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.RIGHT_MOUSE_UP, (server, player, handler, buf, responseSender) -> server.execute(() -> {
            PlayerCapability.IPlayerCapability capability = PlayerCapability.get(player);
            if (capability != null) {
        ...
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.PLAYER_SOLAR_BEAM, (server, player, handler, buf, responseSender) -> server.execute(() -> {
            if (!player.hasStatusEffect(EffectHandler.SUNS_BLESSING)) return;
        ...
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.SCULPTOR_TRADE, (server, player, handler, buf, responseSender) -> {
            ...
            server.execute(() -> {
                Entity entity = player.getWorld().getEntityById(message.getEntityID());
                if (!(entity instanceof EntitySculptor sculptor)) return;
                if (sculptor.getCustomer() != player) return;
                ScreenHandler container = player.currentScreenHandler;
                if (!(container instanceof ContainerSculptorTrade sculptorTrade)) return;
                if (sculptor.checkTestObstructed()) return;
        ...
        ServerPlayNetworking.registerGlobalReceiver(StaticVariables.UMVUTHI_TRADE, (server, player, handler, buf, responseSender) -> {
            ...
            server.execute(() -> {
                Entity entity = player.getWorld().getEntityById(message.getEntityID());
                if (!(entity instanceof EntityUmvuthi umvuthi)) return;
                if (umvuthi.getCustomer() != player) return;
                ScreenHandler container = player.currentScreenHandler;
                if (!(container instanceof ContainerUmvuthiTrade)) return;
                ...
                if (satisfied) {
                    ...
                    satisfied = umvuthi.fulfillDesire(container.getSlot(0));
                    if (satisfied) {
