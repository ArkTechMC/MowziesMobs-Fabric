...
public class IntermittentAnimation<T extends Entity & IntermittentAnimatableEntity> {
    ...
    public float getAnimationFraction(float partialTicks) {
        float interpTimer = this.prevTimeRunning + (this.timeRunning - this.prevTimeRunning) * partialTicks;
        return interpTimer / (float) this.duration;
    }
    ...
    public float getAnimationProgressSinSqrt(float partialTicks) {
        float result = MathHelper.sin(1.57079632679F * this.getAnimationFraction(partialTicks));
        return result * result;
    }
    ...
    public float getAnimationProgressSinToTenWithoutReturn(float partialTicks) {
        if (this.runDirection == -1) {
            return MathHelper.sin(1.57079632679F * this.getAnimationFraction(partialTicks)) * MathHelper.sin(1.57079632679F * this.getAnimationFraction(partialTicks));
        }
        return (float) Math.pow(MathHelper.sin(1.57079632679F * this.getAnimationFraction(partialTicks)), 10);
    }
